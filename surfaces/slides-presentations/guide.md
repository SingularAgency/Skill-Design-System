...
Decks de Singular (kickoffs, propuestas, QBRs, pitch). Marca **corporativa** (rojo por defecto; azul para decks de *producto*). Generación vía **Gamma** (MCP) o **PPTX** (skill `pptx`).
...
| Rol | Valor (corporativo / rojo) |
|---|---|
| Fondo | `#050505` → `#010203` (dark dramático) + fondo de marca opcional (mist + grid) |
| Acento / títulos destacados | `--primary` `#d4513b` + `--gradient-primary` (coral→rojo) |
...
- Acento de marca en **una** palabra/dato por slide — no recargar de rojo.
